...
// ==========================================
// VIDEO LAZY LOADING - PERFORMANCE OPTIMIZATION
// ==========================================
function initVideoLazyLoading() {
    // Intersection Observer ile video lazy loading
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (source && source.dataset.src) {
                    ...
                    // Artık observe etmeye gerek yok
                    videoObserver.unobserve(video);
                }
            }
        });
    }, {
        rootMargin: '50px', // Video görünmeden 50px önce yüklemeye başla
        threshold: 0.1
    });
    
    // Tüm lazy video'ları observe et
    document.querySelectorAll('video[loading="lazy"]').forEach(video => {
        videoObserver.observe(video);
    });
}

// Video lazy loading'i başlat
document.addEventListener('DOMContentLoaded', () => {
    initVideoLazyLoading();
    console.log('Video lazy loading aktif edildi');
});